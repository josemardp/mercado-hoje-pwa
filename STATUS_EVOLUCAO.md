...
**Última atualização:** 11/08/2026 (carry-over de Compras corrigido, linha de item redesenhada no celular e classificador de categoria/emoji reescrito — ver as três últimas linhas da seção 14; o plano dos 4 achados residuais da seção 13 continua sendo o foco da próxima sessão)
...
| 11/08/2026 | Classificação (pós-sprints) | `classifyCategory.ts` reescrito. Antes: emoji vinha de uma tabela por CATEGORIA (todo item de mercearia era 🛒, todo desconhecido 📦) e o casamento era `nome.includes(palavra)`, que classificava "salsicha" como bebida (contém "cha"), "luva" como fruta ("uva"), "aromatizante" como fruta ("roma") e "creme de leite" como limpeza ("creme"). Agora: dicionário único com ~120 entradas (produto → emoji + categoria juntos), casamento por palavra inteira com a expressão mais longa vencendo, tolerância a acento, plural, quantidade/embalagem ("2kg de arroz", "6 latas de cerveja") e ~40 marcas comuns; pistas genéricas antes de cair em "Outros". Itens já salvos com emoji genérico são corrigidos na carga (`upgradeItemClassification`, local e determinístico, sem fila de sync), sem tocar em emoji específico nem em categoria corrigida à mão. Rótulo da categoria `frutas` virou "Hortifrúti" (só o nome exibido; a chave no banco continua `frutas`), que é o que ela sempre conteve | 86 testes aprovados (17 no classificador, cobrindo cada regressão acima); conferido também com duas varreduras de ~150 nomes reais de supermercado e com a lista renderizada no navegador nos dois temas; typecheck, lint e build de produção aprovados | `src/lib/classifyCategory.ts`, `src/lib/useStore.ts`, `src/lib/categories.ts`, `src/App.tsx`, `src/index.css` |

| 11/08/2026 | Classificação — expansão | Dicionário separado do motor (`productDictionary.ts` × `classifyCategory.ts`) e expandido em 4 fases: (1) hortifrúti completo, com as frutas brasileiras que caíam em "Outros" (atemoia, pinha, jaca, pequi, cupuaçu, umbu, sapoti); (2) ~350 termos de marca (H2OH, Coca, Omo, Ypê, Colgate, Tio João, Camil, Nescau, Pedigree); (3) casa/bazar/farmácia não-comida (pinça, cortador de unha, pente, esmalte, extensão, cadeado, preservativo); (4) despensa brasileira (fermento, páprica, mungunzá, tucupi, papinha, pasta de amendoim). Total: 188 entradas, 1.676 termos. Duas melhorias de motor junto: leitura em duas passadas (preserva "saco plástico"/"sachê para gato", que a remoção de palavras de embalagem engolia) e desempate produto > marca ("Camil feijão" é feijão, não arroz) | 96 testes aprovados, incluindo 9 de integridade do dicionário que reprovam termo duplicado, termo com acento, entrada sem emoji e termo inalcançável pelo motor — foram eles que pegaram 10 defeitos reais durante a expansão (juçara, saquê, baré, "51", seleta, "3 corações", parafuso × macarrão, cola × Coca, frutas secas × castanhas). Varredura final de 92 nomes reais: nenhum caiu em 📦. Bundle +6 kB gzip | `src/lib/productDictionary.ts` (novo), `src/lib/classifyCategory.ts`, `src/lib/__tests__/productDictionary.test.ts` (novo) |
